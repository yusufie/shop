...
interface UserState {
  ...
  isLoggedIn: boolean;
  setLoggedIn: (accessToken: string, refreshToken: string, user: Record<string, any>) => void;
  setLoggedOut: () => void;
}

export const useUserStore = create<UserState>((set) => {
  // Check if localStorage is available (not in SSR)
  const storedAccessToken = typeof localStorage !== 'undefined' ? localStorage.getItem('accessToken') : null;
